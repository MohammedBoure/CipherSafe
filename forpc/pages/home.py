...
from utils.BusinessLogic import (DataPreparationTuple,
                                 get_best_match,
                                 convert_data_format,
                                 save_data,
                                 restore_app,
...
def home_view(page):
    if shared.tray_icon is None: 
        image = Image.open("icon.png")
        
        shared.tray_icon = Icon("TestIcon", image, menu=Menu(
            MenuItem("إظهار التطبيق", lambda _: restore_app(page)),
            MenuItem("خروج", lambda _: exit_program(shared.tray_icon, page))
        ))
        
        def run_tray():
            shared.tray_icon.run()
    ...
    buttons = [
        ...
        create_icon_button(ft.Icons.REMOVE,"إخفاء" ,lambda _:minimize_app(_,page)),
        create_icon_button(ft.Icons.LOGOUT, "تسجيل الخروج",lambda _:end_window(page)),
